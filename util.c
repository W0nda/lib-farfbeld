...
void
ff_read_header(uint32_t *width, uint32_t *height, FILE *finput)
{
	uint32_t hdr[4];

	efread(hdr, sizeof(*hdr), LEN(hdr), finput);
...
void
ff_write_header(uint32_t width, uint32_t height, FILE *foutput)
{
	uint32_t tmp;
	...
	tmp = htonl(width);
	efwrite(&tmp, sizeof(tmp), 1, foutput);
